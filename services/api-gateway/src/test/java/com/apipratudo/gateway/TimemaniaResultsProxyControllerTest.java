...
@ActiveProfiles("test")
class TimemaniaResultsProxyControllerTest {

  private static MockWebServer quotaServer;
  private static MockWebServer timemaniaServer;
  ...
  static void registerProperties(DynamicPropertyRegistry registry) {
    ...
    if (timemaniaServer == null) {
      timemaniaServer = new MockWebServer();
      timemaniaServer.setDispatcher(new Dispatcher() {
        @Override
        public MockResponse dispatch(RecordedRequest request) {
          if ("GET".equals(request.getMethod())
              && "/v1/timemania/resultado-oficial".equals(request.getPath())) {
            return new MockResponse()
                .setResponseCode(200)
                .setHeader("Content-Type", "application/json")
      ...
      try {
        timemaniaServer.start();
      } catch (IOException e) {
        throw new IllegalStateException("Failed to start timemania mock server", e);
      }
    }

    registry.add("quota.base-url", () -> quotaServer.url("/").toString());
    registry.add("quota.internal-token", () -> "test-internal");
    registry.add("timemania.base-url", () -> timemaniaServer.url("/").toString());
  }
  ...
  void proxyRetornaResultado() throws Exception {
    ...
    RecordedRequest recorded = timemaniaServer.takeRequest(1, TimeUnit.SECONDS);
    assertThat(recorded).isNotNull();
    assertThat(recorded.getMethod()).isEqualTo("GET");
    assertThat(recorded.getPath()).isEqualTo("/v1/timemania/resultado-oficial");
  }
}
