...
@ActiveProfiles("test")
class QuinaResultsProxyControllerTest {

  private static MockWebServer quotaServer;
  private static MockWebServer quinaServer;
  ...
  static void registerProperties(DynamicPropertyRegistry registry) {
    ...
    if (quinaServer == null) {
      quinaServer = new MockWebServer();
      quinaServer.setDispatcher(new Dispatcher() {
        @Override
        public MockResponse dispatch(RecordedRequest request) {
          if ("GET".equals(request.getMethod())
              && "/v1/quina/resultado-oficial".equals(request.getPath())) {
            return new MockResponse()
                .setResponseCode(200)
                .setHeader("Content-Type", "application/json")
      ...
      try {
        ...
        throw new IllegalStateException("Failed to start quina mock server", e);
      }
    }

    registry.add("quota.base-url", () -> quotaServer.url("/").toString());
    registry.add("quota.internal-token", () -> "test-internal");
    registry.add("quina.base-url", () -> quinaServer.url("/").toString());
  }
  ...
  void proxyRetornaResultado() throws Exception {
    ...
    RecordedRequest recorded = quinaServer.takeRequest(1, TimeUnit.SECONDS);
    assertThat(recorded).isNotNull();
    assertThat(recorded.getMethod()).isEqualTo("GET");
    assertThat(recorded.getPath()).isEqualTo("/v1/quina/resultado-oficial");
  }
}
