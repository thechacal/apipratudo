...
@ActiveProfiles("test")
class DiadesorteResultsProxyControllerTest {

  private static MockWebServer quotaServer;
  private static MockWebServer diadesorteServer;
  ...
  static void registerProperties(DynamicPropertyRegistry registry) {
    ...
    if (diadesorteServer == null) {
      diadesorteServer = new MockWebServer();
      diadesorteServer.setDispatcher(new Dispatcher() {
        @Override
        public MockResponse dispatch(RecordedRequest request) {
          if ("GET".equals(request.getMethod())
              && "/v1/diadesorte/resultado-oficial".equals(request.getPath())) {
            return new MockResponse()
                .setResponseCode(200)
                .setHeader("Content-Type", "application/json")
      ...
      try {
        diadesorteServer.start();
      } catch (IOException e) {
        throw new IllegalStateException("Failed to start diadesorte mock server", e);
      }
    }

    registry.add("quota.base-url", () -> quotaServer.url("/").toString());
    registry.add("quota.internal-token", () -> "test-internal");
    registry.add("diadesorte.base-url", () -> diadesorteServer.url("/").toString());
  }
  ...
  void proxyRetornaResultado() throws Exception {
    ...
    RecordedRequest recorded = diadesorteServer.takeRequest(1, TimeUnit.SECONDS);
    assertThat(recorded).isNotNull();
    assertThat(recorded.getMethod()).isEqualTo("GET");
    assertThat(recorded.getPath()).isEqualTo("/v1/diadesorte/resultado-oficial");
  }
}
