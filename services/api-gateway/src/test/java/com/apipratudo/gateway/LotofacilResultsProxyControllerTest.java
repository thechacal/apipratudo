...
@ActiveProfiles("test")
class LotofacilResultsProxyControllerTest {

  private static MockWebServer quotaServer;
  private static MockWebServer lotofacilServer;
  ...
  static void registerProperties(DynamicPropertyRegistry registry) {
    ...
    if (lotofacilServer == null) {
      lotofacilServer = new MockWebServer();
      lotofacilServer.setDispatcher(new Dispatcher() {
        @Override
        public MockResponse dispatch(RecordedRequest request) {
          if ("GET".equals(request.getMethod())
              && "/v1/lotofacil/resultado-oficial".equals(request.getPath())) {
            return new MockResponse()
                .setResponseCode(200)
                .setHeader("Content-Type", "application/json")
                .setBody("{" +
                    ...
                    "\"dezenas\":[\"01\",\"03\",\"04\",\"05\",\"07\",\"08\",\"10\",\"11\",\"12\",\"13\",\"17\",\"18\",\"19\",\"22\",\"25\"]," +
      ...
      try {
        lotofacilServer.start();
      } catch (IOException e) {
        throw new IllegalStateException("Failed to start lotofacil mock server", e);
      }
    }

    registry.add("quota.base-url", () -> quotaServer.url("/").toString());
    registry.add("quota.internal-token", () -> "test-internal");
    registry.add("lotofacil.base-url", () -> lotofacilServer.url("/").toString());
  }
  ...
  void proxyRetornaResultado() throws Exception {
    ...
    RecordedRequest recorded = lotofacilServer.takeRequest(1, TimeUnit.SECONDS);
    assertThat(recorded).isNotNull();
    assertThat(recorded.getMethod()).isEqualTo("GET");
    assertThat(recorded.getPath()).isEqualTo("/v1/lotofacil/resultado-oficial");
  }
}
